...
var util = {
  parseWherePhrase: function parseWherePhrase(options) {
    ...
    if (keys.length > 1) {
      return keys
        .map(function (key) {
          return parseWherePhrase({
            tableAs,
            where: { [key]: waterlineWhere[key] },
          });
        })
        .join(" AND ");
    } else {
      ...
      if (leftHandSide === "or" && _.isArray(rightHandSide)) {
        return `((${rightHandSide
          .map(function (item) {
            return util.parseWherePhrase({ tableAs, where: item });
          })
          .join(") OR (")}))`;
      }
      ...
      if (_.isObject(rightHandSide)) {
        if (_.isArray(rightHandSide) || _.has(rightHandSide, "in")) {
          ...
          var valuesWithoutNull = _.filter(rightHandSide, (value) => {
            return !_.isNull(value);
          });
          if (hadNull) {
            return `(([${tableAs}].[${leftHandSide}] IS NULL) OR ([${tableAs}].[${leftHandSide}] IN (${util.prepareValue(
              { value: valuesWithoutNull }
            )})))`;
          } else {
            return `[${tableAs}].[${leftHandSide}] IN (${util.prepareValue({
              value: valuesWithoutNull,
            })})`;
          }
        }
        if (_.has(rightHandSide, "<")) {
          return `[${tableAs}].[${leftHandSide}] < ${util.prepareValue({
            value: rightHandSide["<"],
          })}`;
        }
        if (_.has(rightHandSide, "lessThan")) {
          return `[${tableAs}].[${leftHandSide}] < ${util.prepareValue({
            value: rightHandSide["lessThan"],
          })}`;
        }
        if (_.has(rightHandSide, "<=")) {
          return `[${tableAs}].[${leftHandSide}] <= ${util.prepareValue({
        ...
        if (_.has(rightHandSide, "lessThanOrEqual")) {
          return `[${tableAs}].[${leftHandSide}] <= ${util.prepareValue({
            value: rightHandSide["lessThanOrEqual"],
          })}`;
        }
        if (_.has(rightHandSide, ">")) {
          return `[${tableAs}].[${leftHandSide}] > ${util.prepareValue({
        ...
        if (_.has(rightHandSide, "greaterThan")) {
          return `[${tableAs}].[${leftHandSide}] > ${util.prepareValue({
            value: rightHandSide["greaterThan"],
          })}`;
        }
        if (_.has(rightHandSide, ">=")) {
          return `[${tableAs}].[${leftHandSide}] >= ${util.prepareValue({
        ...
        if (_.has(rightHandSide, "greaterThanOrEqual")) {
          return `[${tableAs}].[${leftHandSide}] >= ${util.prepareValue({
            value: rightHandSide["greaterThanOrEqual"],
          })}`;
        }
        if (_.has(rightHandSide, "!=") && _.isArray(rightHandSide["!="])) {
          ...
          if (hadNull) {
            return `(([${tableAs}].[${leftHandSide}] IS NOT NULL) AND ([${tableAs}].[${leftHandSide}] NOT IN (${util.prepareValue(
              { value: valuesWithoutNull }
            )})))`;
          } else {
            return `[${tableAs}].[${leftHandSide}] NOT IN (${util.prepareValue({
              value: valuesWithoutNull,
            })})`;
          }
        }
        if (_.has(rightHandSide, "!=")) {
          if (
            `${util.prepareValue({ value: rightHandSide["!="] })}` === "null"
          ) {
        ...
        if (_.has(rightHandSide, "not")) {
          if (
            `${util.prepareValue({ value: rightHandSide["not"] })}` === "null"
          ) {
            return `[${tableAs}].[${leftHandSide}] is not null`;
          } else {
            return `[${tableAs}].[${leftHandSide}] <> ${util.prepareValue({
              value: rightHandSide["not"],
            })}`;
          }
        }
        if (_.has(rightHandSide, "like")) {
          return `[${tableAs}].[${leftHandSide}] LIKE ${util.prepareValue({
            value: rightHandSide.like,
          })}`;
        }
        if (_.has(rightHandSide, "contains")) {
          return `[${tableAs}].[${leftHandSide}] LIKE ${util.prepareValue({
            value: `%${rightHandSide.contains}%`,
          })}`;
        }
        if (_.has(rightHandSide, "startsWith")) {
          return `[${tableAs}].[${leftHandSide}] LIKE ${util.prepareValue({
            value: `${rightHandSide.startsWith}%`,
          })}`;
        }
        if (_.has(rightHandSide, "endsWith")) {
          return `[${tableAs}].[${leftHandSide}] LIKE ${util.prepareValue({
            value: `%${rightHandSide.endsWith}`,
          })}`;
        }
      } else {
...
var adapter = {
  ...
  registerDatastore: async function (
    ...
    */
    ...
    var urlMatches = datastoreConfig.url.match(
      /^mssql:\/\/([^:]+):([^@]+)@([^/:]+)(?:(?::(\d+))?\/([^/]+)|\/([^/]+)\/([^/]+))$/
    );
    if (!urlMatches) {
      return done(
        new Error(
          "Invalid configuration for datastore `" +
            datastoreName +
            "`:  `url` definition does not follow the pattern"
        )
      );
    }
    ...
    registeredDatastores[datastoreName] = {
      ...
      manager: (
        await mssqlDriver.createManager({
  ...
  count: async function (datastoreName, query, cb) {
    try {
      ...
      utils.convertDates({
        attributes: dsEntry.dbSchema[tableName].attributes,
        where: criteria.where,
      });

      criteria.__primaryKey__ = adapter.getPrimaryKeyColumnName(
        datastoreName,
        tableName
      );
      var schemaName = getSchemaName(datastoreName, tableName);
      ...
      let recordset = await adapter.connectAndQuery({
        query: statement,
        datastoreName,
        meta: query.meta,
      });
  ...
  create: async function (
    datastoreName,
    { newRecord, using: tableName, meta, method },
    cb
  ) {
    try {
      ...
      let results;
      try {
        results = await adapter.connectAndQuery({
          query: statement,
          datastoreName,
          meta,
        });
      ...
      var _query = new Query(
        registeredDatastores[datastoreName].dbSchema[tableName].attributes
      );
  ...
  update: async function (datastoreName, query = {}, cb) {
    try {
      ...
      var isJunctionTable =
        registeredDatastores[datastoreName].dbSchema[tableName].meta &&
        registeredDatastores[datastoreName].dbSchema[tableName].meta
          .junctionTable;
      var schemaName = getSchemaName(datastoreName, tableName);
      ...
      var statement =
        "SELECT [" + pk + "] FROM" + wrappedTableName + " " + criteria;
      ...
      if (isJunctionTable && isValueNull && query.criteria.where) {
        ...
        if (serializedOptions.toLowerCase().indexOf("where") < 0) {
          return cb(
            new Error("where clause is required during join table delete")
          );
        }
        ...
        statement =
          ...
          wrappedTableName +
          " SET " +
          sql.updateCriteria(tableName, values) +
          " ";
        serializedOptions = sql.serializeOptions(tableName, query.criteria);
        statement += serializedOptions;
      }

      await adapter.connectAndQuery({
        query: statement,
        datastoreName,
        meta: query.meta,
      });
  ...
  connectAndQuery: async function ({ query, datastoreName, meta = {} } = {}) {
    ...
    if (!connection) {
      connection = (
        await registeredDatastores[datastoreName].driver.getConnection({
          manager: registeredDatastores[datastoreName].manager,
        })
      ).connection;
    }
    const report = await registeredDatastores[
      datastoreName
  ...
  join: async function (datastoreName, query, cb) {
    try {
      ...
      if (!registeredDatastores[datastoreName].manager.version) {
        ...
        registeredDatastores[datastoreName].manager.version = Number(
          Object.values(report.result.recordsets[0][0])[0].split(".")[0]
        );
      }
      ...
      if (!registeredDatastores[datastoreName].manager.edition) {
        ...
        registeredDatastores[datastoreName].manager.edition = String(
          Object.values(report.result.recordsets[0][0])[0]
        );
      }
      // if sql 2014 or lower, and not SQL Azure, FOR JSON not supported, so run old join code
      if (
        registeredDatastores[datastoreName].manager.version < 13 &&
        registeredDatastores[datastoreName].manager.edition !== "SQL Azure"
      ) {
      ...
      var toManies = _.groupBy(
        _.filter(query.joins, { collection: true }),
        "alias"
      );
      ...
      attrsWeCareAbout.forEach(function (attrName) {
        ...
        if (toOne) {
          ...
          if (
            toOne.criteria &&
            toOne.criteria.where &&
            !_.isEmpty(toOne.criteria.where)
          ) {
            whereParts.push(
              util.parseWherePhrase({
                tableAs: toOne.parentKey,
                where: toOne.criteria.where,
              })
            );
          }

          joinParts.push(
            `left outer join [${toOne.child}] as [${toOne.parentKey}] on [${toOne.parentKey}].[${toOne.childKey}] = [${toOne.parent}].[${toOne.parentKey}]`
          );
          if (selectCriteria) {
            // console.log('selectCriteria: ', selectCriteria)
            var childAttrs =
              registeredDatastores[datastoreName].dbSchema[toOne.child]
                .attributes;
            _.uniq(selectCriteria).forEach(function (selectItem) {
              var childAttr =
                childAttrs[selectItem] ||
                _.find(childAttrs, { columnName: selectItem });
              if (!childAttr.hasOwnProperty("collection")) {
                // && !childAttr.hasOwnProperty('model')) {
                selectParts.push(
                  `[${toOne.parentKey}].[${selectItem}] as [${toOne.parentKey}.${selectItem}]`
                );
              }
            });
          }
        ...
        if (toMany) {
          ...
          if (toMany.length === 1) {
            ...
            if (
              toMany.criteria &&
              toMany.criteria.where &&
              !_.isEmpty(toMany.criteria.where)
            ) {
              whereFromCriteriaClause = `AND ${util.parseWherePhrase({
                tableAs: toMany.child,
                where: toMany.criteria.where,
              })}`;
            }
          ...
          var childAttrs =
            registeredDatastores[datastoreName].dbSchema[toMany.child]
              .attributes;
          _.uniq(selectCriteria).forEach(function (selectItem) {
            var childAttr;
            var childAttrKey;
          ...
          var limitClause = "";
          var skipClause = "";
          var orderByClause = util.parseOrderByPhrase({
            tableAs: toMany.child,
            sort: toMany.criteria.sort,
          });
          ...
          if (toMany.criteria.skip) {
            ...
            if (!orderByClause) {
              ...
              var primaryKey = adapter.getPrimaryKeyColumnName(
                datastoreName,
                toMany.child
              );

              orderByClause = util.parseOrderByPhrase({
                tableAs: toMany.child,
                sort: { [primaryKey]: 1 },
              });
            }
        ...
        if (!isPopulation && !attr.hasOwnProperty("collection")) {
          ...
          const thisIsJoin = _.find(query.joins, {
            parentKey: attr.columnName,
          });
      ...
      if (
        criteria.where &&
        !_.isEmpty(criteria.where) &&
        _.isObject(criteria.where)
      ) {
        whereParts.unshift(
          util.parseWherePhrase({ tableAs: tableName, where: criteria.where })
        );
      }
      ...
      try {
        let results = await adapter.connectAndQuery({
          query: queryAsString,
          datastoreName,
          meta: query.meta,
        });
          ...
          _.each(query.joins, (join) => {
            if (join.collection === true) {
              ...
              if (!parsedResult.hasOwnProperty(join.alias)) {
                ...
                // should we normalize these? I think they may already be normalized...
                return (parsedResult[join.alias] = normalizeResults(
                  datastoreName,
                  join.child,
                  parsedResult[join.alias]
                ));
              }
            }
            if (typeof parsedResult[join.parentKey] === "undefined") {
              ...
              return (parsedResult[join.parentKey] = normalizeResults(
                datastoreName,
                join.child,
                parsedResult[join.parentKey]
              ));
            }
          });
        });

        parsedResults = normalizeResults(
          datastoreName,
          tableName,
          parsedResults
        );
  ...
  oldJoin: async function (datastoreName, wlQuery, cb) {
    ...
    promises[tableName] = adapter.find(datastoreName, {
      using: tableName,
      criteria,
    });
        ...
        return Promise.all(
          _.map(joins, function (join) {
            promises[join.child] = promises[join.parent].then(async function (
              ...
              var attributes =
                registeredDatastores[datastoreName].dbSchema[join.child]
                  .attributes;
              ...
              var tableNameWithSchema =
                "[" + schemaName + "]" + ".[" + join.child + "]";
              var orderBy = "";
              var sqlQueryParts = _.map(parentResults, function (parentResult) {
                var criteria = _.clone(
                  join.criteria || {
                    where: {},
                  }
                );
                ...
                if (!join.junctionTable) {
                  ...
                  criteria.where[join.middleLeftKey] =
                    parentResult[join.parentKey];
                }
                  ...
                  if (join.select && join.select.length) {
                    _.each(join.select, function (selectItem) {
                      ...
                      if (
                        ...
                        "collection" in attributes[selectItem]
                      ) {
                ...
                var generatedSelectQuery = sql.selectQuery(
                  tableNameWithSchema,
                  criteria
                );
                if (criteria.skip) {
                  return generatedSelectQuery.replace(
                    /ORDER BY(?!.*\)).*$/,
                    ""
                  );
                }
              ...
              let childResults = await adapter.connectAndQuery({
                query: sqlQuery,
                datastoreName,
                meta: wlQuery.meta,
              });
              ...
              childResults.forEach((childResult) => {
                _.each(attributes, (attribute, attributeAlias) => {
                  if (
                    ...
                    childResult.hasOwnProperty(attribute.columnName)
                  ) {
                    if (attributeAlias !== attribute.columnName) {
                      ...
                      childResult[attributeAlias] =
                        childResult[attribute.columnName];
              ...
              if (join.junctionTable) {
                _.each(groupedResults, function (arrayOfChildrenWithJunction) {
                  _.each(
                    arrayOfChildrenWithJunction,
                    function (childWithJunction) {
              ...
              if (childResults.length) {
                if (join.model) {
                  _.each(topLevelResults, function (topLevelResult) {
                    topLevelResult[join.alias] = _.cloneDeep(
                      (groupedResults[topLevelResult[join.parentKey]] &&
                        groupedResults[topLevelResult[join.parentKey]][0]) ||
                        null
                    );
                    // groupedResults[topLevelResult[join.parentKey]] ||
                    // [];
                  ...
                  _.each(topLevelResults, function (topLevelResult) {
                    if (
                      typeof groupedResults[topLevelResult[join.parentKey]] !==
                      "undefined"
                    ) {
                      topLevelResult[join.alias] = _.cloneDeep(
                        groupedResults[topLevelResult[join.parentKey]]
...
function normalizeResults(
  ...
  if (_.isArray(recordset)) {
    return _.map(recordset, (row) => {
      return normalizeResults(datastoreName, tableName, row, {
        preserveColumns,
      });
    });
  }
  ...
  _.each(tableAttributes, (attrDefinition, attrName) => {
    ...
    if (!columnName && recordset.hasOwnProperty(attrName)) {
      // it's not stored here. it's an association.
    ...
    if (_.has(recordset, columnName)) {
      ...
      if (
        attrDefinition.autoMigrations &&
        attrDefinition.autoMigrations.columnType &&
        attrDefinition.autoMigrations.columnType.indexOf("date") === 0
      ) {
        return (output[columnName] = moment(value)
          .utc()
